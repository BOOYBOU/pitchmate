...
export const AuthView: React.FC = () => {
  ...
  const [googlePickerAction, setGooglePickerAction] = useState<'signin' | 'signup'>('signin');
  ...
  // Google OAuth Flow Handler (Native account picker or device dialog)
  const handleGoogleAuth = async (action: 'signin' | 'signup') => {
    ...
    try {
      ...
      if (res.code === 'SHOW_GOOGLE_PICKER') {
        // Open the native-looking account selection dialog
        setGooglePickerAction(action);
      ...
      // If popup blocked or error, fallback to Account Chooser
      setGooglePickerAction(action);
      setShowGooglePickerModal(true);
    } finally {
  ...
  // Account Chooser Account Selected
  const handleSelectGoogleAccount = async (account: { email: string; name: string; avatarUrl?: string }) => {
    ...
    try {
      const res = await loginWithGoogle(googlePickerAction, {
        email: account.email,
        name: account.name,
      ...
      if (!res.success) {
        if (googlePickerAction === 'signin') {
          setSignInError(res.error || (language === 'ar' ? 'هذا الحساب غير مسجل أو معلق.' : 'Account not found or pending.'));
        } else {
          setSignUpError(res.error || (language === 'ar' ? 'هذا الحساب مسجل بالفعل.' : 'Account already exists.'));
        }
      } else if (res.pendingApproval) {
        if (res.user) {
          setRegisteredUserName(res.user.name);
      ...
      if (googlePickerAction === 'signin') {
        setSignInError(language === 'ar' ? 'حدث خطأ أثناء تسجيل الدخول.' : 'Sign in error.');
      } else {
        setSignUpError(language === 'ar' ? 'حدث خطأ أثناء إنشاء الحساب.' : 'Sign up error.');
      }
    } finally {
      setGoogleLoading(false);
    }
  ...
  return (
    <div className="min-h-screen w-full bg-[#040D09] text-white flex flex-col justify-between relative overflow-hidden font-sans select-none antialiased">
        ...
        <div className="w-full max-w-md bg-[#082218]/90 backdrop-blur-xl border border-[#E5B869]/30 rounded-3xl p-6 sm:p-8 shadow-2xl shadow-emerald-950/60 space-y-6 text-center">
          ...
          {mode === 'signin' && (
            <div className="space-y-4 text-xs">
              ...
              {/* Google Native One-Click Sign In Button */}
              <button
                id="auth-google-signin-btn"
                type="button"
                onClick={() => handleGoogleAuth('signin')}
                disabled={googleLoading || isSubmitting}
                className="w-full py-3 px-4 bg-[#082218] hover:bg-[#0E382A] text-white border border-[#E5B869]/40 hover:border-[#E5B869] rounded-xl font-bold transition-all flex items-center justify-center gap-3 cursor-pointer shadow-md hover:scale-[1.01] active:scale-[0.99] text-xs sm:text-sm disabled:opacity-50"
              >
                ...
                <span className="font-display">
                  {googleLoading && googlePickerAction === 'signin'
                    ? (language === 'ar' ? 'جاري الاتصال بـ Google...' : 'Connecting to Google...')
                    : (language === 'ar' ? 'متابعة الدخول عبر حساب Google' : 'Sign in with Google Account')}
                </span>
              </button>
          ...
          {mode === 'signup' && (
            <div className="space-y-4 text-xs">
              ...
              {/* Google Native One-Click Sign Up Button */}
              <button
              ...
              >
                ...
                <span className="font-display">
                  {googleLoading && googlePickerAction === 'signup'
                    ? (language === 'ar' ? 'جاري الاتصال بـ Google...' : 'Connecting to Google...')
                    : (language === 'ar' ? 'إنشاء حساب فوري عبر Google' : 'Create Account with Google')}
                </span>
              </button>
      ...
      {/* Google Native Account Chooser Dialog (Google Sign-In Popup) */}
      {showGooglePickerModal && (
        ...
        >
          <div
            className="w-full max-w-md bg-[#071913] border-2 border-[#E5B869] rounded-3xl p-6 shadow-2xl space-y-5 text-white animate-in zoom-in-95 duration-200 relative"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="flex items-center justify-between pb-3 border-b border-[#E5B869]/25">
              <div className="flex items-center gap-3">
                ...
                <div className="text-start">
                  <h3 className="text-sm font-black text-white font-display">
                    {language === 'ar' ? 'اختيار حساب Google للمتابعة' : 'Choose a Google Account'}
                  </h3>
                  <p className="text-[11px] text-emerald-300/70">
                    {language === 'ar' ? 'للمتابعة إلى تطبيق PitchMate Pro' : 'To continue to PitchMate Pro'}
                  </p>
                </div>
              </div>
              <button
                type="button"
                onClick={() => setShowGooglePickerModal(false)}
                className="p-1.5 rounded-xl text-emerald-400/60 hover:text-white hover:bg-[#0E382A] transition-colors cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>

            {/* List of Device Google Accounts */}
            <div className="space-y-2.5">
              <div className="text-[11px] font-bold text-[#F5D794] uppercase tracking-wider text-start">
                {language === 'ar' ? 'الحسابات المتوفرة على هذا الجهاز:' : 'Accounts on this device:'}
              </div>

              {/* Account 1: Super Admin / Main Google Account */}
              <button
                type="button"
                onClick={() =>
                ...
                className="w-full p-3 rounded-2xl bg-[#0B2A1E] hover:bg-[#0E382A] border border-[#E5B869]/40 hover:border-[#E5B869] transition-all flex items-center justify-between text-start cursor-pointer group shadow-sm hover:scale-[1.01]"
              >
                <div className="flex items-center gap-3">
                  <img
                    src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80"
                    alt="Mustapha"
                    className="w-10 h-10 rounded-full object-cover border-2 border-[#E5B869]"
                  />
                  <div>
                    <div className="flex items-center gap-1.5">
                      <span className="text-xs font-bold text-white group-hover:text-[#F5D794]">
                        Mustapha Bouhbous
                      </span>
                      <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-[#E5B869] text-slate-950 font-black">
                        Admin
                      </span>
                    </div>
                ...
                <div className="w-6 h-6 rounded-full bg-[#04130D] border border-[#E5B869]/30 flex items-center justify-center text-[#F5D794]">
                  {isRTL ? <ArrowLeft className="w-3.5 h-3.5" /> : <ArrowRight className="w-3.5 h-3.5" />}
                </div>
              </button>

              {/* Account 2: Player Yassine Google Account */}
              <button
                type="button"
                onClick={() =>
                  handleSelectGoogleAccount({
                    email: 'yassine.bennani@gmail.com',
                    name: 'Yassine Bennani',
                    avatarUrl: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&auto=format&fit=crop&q=80',
                  })
                }
                className="w-full p-3 rounded-2xl bg-[#0B2A1E] hover:bg-[#0E382A] border border-[#E5B869]/30 hover:border-[#E5B869] transition-all flex items-center justify-between text-start cursor-pointer group shadow-sm hover:scale-[1.01]"
              >
                <div className="flex items-center gap-3">
                  <img
                    src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&auto=format&fit=crop&q=80"
                    alt="Yassine"
                    className="w-10 h-10 rounded-full object-cover border border-[#E5B869]/50"
                  />
                  <div>
                    <span className="text-xs font-bold text-white group-hover:text-[#F5D794] block">
                      Yassine Bennani
                    </span>
                    <span className="text-[11px] text-emerald-300 font-mono">
                      yassine.bennani@gmail.com
                    </span>
                  </div>
                </div>
                <div className="w-6 h-6 rounded-full bg-[#04130D] border border-[#E5B869]/30 flex items-center justify-center text-[#F5D794]">
              ...
              {/* Account 3: Player Reda Google Account */}
              <button
                type="button"
                onClick={() =>
                  handleSelectGoogleAccount({
                    email: 'reda.elamrani@gmail.com',
                    name: 'Reda El Amrani',
            ...
            <div className="pt-3 border-t border-[#E5B869]/20 space-y-3">
              <div className="text-[11px] font-bold text-emerald-300 text-start flex items-center gap-1.5">
                <Plus className="w-3.5 h-3.5 text-[#E5B869]" />
                <span>{language === 'ar' ? 'أو استخدم حساب Google آخر:' : 'Or enter another Google account:'}</span>
              </div>

              <div className="space-y-2">
                <input
                  type="text"
                  placeholder={language === 'ar' ? 'الاسم الكامل (مثال: كريم العلوي)' : 'Full Name (e.g. Karim Alaoui)'}
                  value={customGoogleName}
                  onChange={(e) => setCustomGoogleName(e.target.value)}
                  className="w-full px-3 py-2 bg-[#04130D] border border-[#E5B869]/30 rounded-xl text-xs text-white placeholder-emerald-400/40 focus:outline-none focus:border-[#E5B869]"
                />
                <input
                  type="email"
                  placeholder={language === 'ar' ? 'عنوان Gmail (مثال: name@gmail.com)' : 'Gmail address (e.g. name@gmail.com)'}
                ...
                <button
                  ...
                  className="w-full py-2.5 bg-gradient-to-r from-[#F5D794] via-[#E5B869] to-[#C69238] hover:opacity-90 disabled:opacity-40 text-slate-950 rounded-xl font-bold text-xs transition-all cursor-pointer shadow-md"
                >
                  {language === 'ar' ? 'المتابعة بهذا الحساب' : 'Continue with this Account'}
                </button>
              </div>
            </div>
          </div>
