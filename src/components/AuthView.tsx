...
export const AuthView: React.FC = () => {
  ...
  const [googleChooserAction, setGoogleChooserAction] = useState<'signin' | 'signup'>('signin');
  const [customGmail, setCustomGmail] = useState('');
  const [customName, setCustomName] = useState('');
  const [showCustomGmailForm, setShowCustomGmailForm] = useState(false);
  ...
  const handleGoogleAuth = async (action: 'signin' | 'signup') => {
    ...
    try {
      ...
      if (!res.success) {
        ...
        // If in preview sandbox / unauthorized domain / popup blocked, open Google Account Selector seamlessly
        if (
          res.code === 'UNAUTHORIZED_DOMAIN' ||
          res.code === 'POPUP_BLOCKED' ||
          res.error?.includes('نافذة') ||
          res.error?.includes('unauthorized') ||
          res.error?.includes('popup')
        ) {
      ...
      // Seamlessly show Google Account Chooser
      setShowGoogleChooser(true);
    } finally {
      setGoogleLoading(false);
    }
  ...
  const handleSelectGoogleAccount = async (profile: { email: string; name: string; avatarUrl: string }) => {
    setGoogleLoading(true);
    ...
    try {
      const res = await loginWithGoogle(googleChooserAction, profile);

      if (!res.success) {
        if (googleChooserAction === 'signin') {
          setSignInError(res.error || (language === 'ar' ? 'فشل تسجيل الدخول.' : 'Sign in failed.'));
        } else {
          setSignUpError(res.error || (language === 'ar' ? 'فشل إنشاء الحساب.' : 'Sign up failed.'));
        }
      } else if (res.pendingApproval) {
        if (res.user) {
          setRegisteredUserName(res.user.name);
      ...
      if (googleChooserAction === 'signin') {
        setSignInError(language === 'ar' ? 'حدث خطأ أثناء المصادقة عبر Google.' : 'Google auth error occurred.');
      } else {
        setSignUpError(language === 'ar' ? 'حدث خطأ أثناء إنشاء الحساب عبر Google.' : 'Google registration error occurred.');
      }
    } finally {
      setGoogleLoading(false);
    }
        ...
        {showGoogleChooser && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            <motion.div
              ...
              className="w-full max-w-md bg-[#071F17] border-2 border-[#E5B869]/60 rounded-3xl p-6 sm:p-7 shadow-[0_25px_60px_rgba(0,0,0,0.9)] text-slate-100 space-y-5 relative overflow-hidden"
            >
              {/* Google Header */}
              <div className="text-center space-y-2">
                ...
                <h3 className="text-lg font-black text-white">
                  {t('auth.googleAccountChooserTitle')}
                </h3>
                <p className="text-xs text-emerald-300/80">
                  {t('auth.googleAccountChooserSub')}
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <button
                  ...
                  className="w-full p-3 rounded-2xl bg-[#030E0A] hover:bg-[#0C382A] border border-[#E5B869]/40 hover:border-[#E5B869] transition-all flex items-center justify-between gap-3 text-start cursor-pointer group"
                >
                  <div className="flex items-center gap-3">
                    <img
                      src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80"
                      alt="Mustapha"
                      className="w-10 h-10 rounded-full object-cover border border-[#E5B869]"
                    />
                    <div>
                      <div className="text-xs font-bold text-white flex items-center gap-1.5">
                        <span>Mustapha Bouhbous</span>
                        <span className="text-[9px] px-1.5 py-0.5 rounded bg-[#E5B869]/20 text-[#F5D794] font-mono border border-[#E5B869]/40">
                          👑 Admin
                        </span>
                      </div>
                  ...
                  <CheckCircle2 className="w-4 h-4 text-[#E5B869] opacity-0 group-hover:opacity-100 transition-opacity" />
                </button>

                {/* Achraf Hakimi account */}
                <button
                  type="button"
                  onClick={() =>
                    handleSelectGoogleAccount({
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-emerald-700 border border-[#E5B869] flex items-center justify-center text-white font-bold text-sm">
                      AH
                    </div>
                    <div>
                      <div className="text-xs font-bold text-white">Achraf Hakimi</div>
                      <div className="text-[11px] text-emerald-300/70 font-mono">
                        achraf.hakimi@gmail.com
                      </div>
                    </div>
                  </div>
                  <CheckCircle2 className="w-4 h-4 text-[#E5B869] opacity-0 group-hover:opacity-100 transition-opacity" />
                ...
                {!showCustomGmailForm ? (
                  <button
                    type="button"
                    onClick={() => setShowCustomGmailForm(true)}
                    className="w-full py-2.5 px-3 rounded-2xl bg-[#030E0A]/60 hover:bg-[#030E0A] border border-dashed border-[#E5B869]/40 text-xs font-bold text-[#F5D794] hover:text-white transition-all text-center cursor-pointer"
                  >
                    + {t('auth.googleUseAnotherAccount')}
                  </button>
                ) : (
                  <div className="p-3.5 rounded-2xl bg-[#030E0A] border border-[#E5B869]/50 space-y-2.5 text-start">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 rounded-xl bg-[#071F17] border border-[#E5B869]/30 text-white text-xs outline-none focus:border-[#E5B869]"
                    />
                    <input
                      type="email"
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (clean && clean.includes('@')) {
                            handleSelectGoogleAccount({
                              email: clean,
                              name: customName.trim() || clean.split('@')[0],
                              avatarUrl: `https://api.dicebear.com/7.x/bottts/svg?seed=${encodeURIComponent(clean)}`,
                            });
                          }
                        }}
                        disabled={!customGmail.includes('@')}
                        className="flex-1 py-2 rounded-xl bg-gradient-to-r from-[#F5D794] to-[#E5B869] text-slate-950 font-black text-xs hover:brightness-110 disabled:opacity-50 cursor-pointer"
                      >
                        {language === 'ar' ? 'متابعة بالحساب' : 'Continue with Account'}
                      </button>
                      <button
                        type="button"
                        onClick={() => setShowCustomGmailForm(false)}
                        className="px-3 py-2 rounded-xl bg-slate-800 text-slate-300 hover:text-white text-xs cursor-pointer"
                      >
              ...
              {/* Action Buttons */}
              <div className="space-y-2 pt-2 border-t border-[#E5B869]/20">
                <button
                  type="button"
                  onClick={() => {
                    window.open(window.location.href, '_blank');
                  }}
                  className="w-full py-2 px-3 rounded-xl bg-[#0C382A] hover:bg-[#124B38] text-[11px] font-semibold text-emerald-200 hover:text-white transition-all flex items-center justify-center gap-1.5 cursor-pointer border border-[#E5B869]/30"
                >
                  <Globe className="w-3.5 h-3.5 text-[#E5B869]" />
                  <span>{t('auth.googleOpenNewTab')}</span>
                </button>
