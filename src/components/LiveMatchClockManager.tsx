...
export const LiveMatchClockManager: React.FC<LiveMatchClockManagerProps> = ({ match }) => {
  ...
  const [cardReason, setCardReason] = useState('Tactical foul / Reckless challenge');
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold uppercase tracking-wider text-slate-300">
              {isRunning ? 'Match In Progress' : 'Match Clock Paused'}
            </span>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="px-5 py-2 rounded-2xl bg-black/60 border border-slate-700 text-center font-mono">
              ...
              <span className="text-[10px] text-slate-400 block">Min {currentMatchMinute}' / {match.durationMinutes}'</span>
            </div>
          ...
          <div className="text-center space-y-1">
            ...
            <div className="flex items-center justify-center gap-1">
              <button
                ...
                className="w-6 h-6 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold text-xs"
              >
                -
              </button>
              <button
                onClick={() => updateMatchScore(match.id, scoreGreen + 1, scoreBlue)}
                className="w-6 h-6 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs"
              >
                +
              </button>
            </div>
          ...
          <div className="text-center space-y-1">
            ...
            <div className="flex items-center justify-center gap-1">
              ...
              <button
                ...
                className="w-6 h-6 rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs"
              >
                +
              </button>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <form onSubmit={handleAddGoal} className="p-4 rounded-2xl bg-[#090D16] border border-[#1E293B] space-y-3">
          ...
          >
            ...
            Record Goal ({currentMatchMinute}')
          </button>
        </form>
        ...
        <form onSubmit={handleAddSubstitution} className="p-4 rounded-2xl bg-[#090D16] border border-[#1E293B] space-y-3">
          <div className="flex items-center gap-2 text-xs font-bold text-white">
            ...
            <span>Tactical Substitution</span>
          </div>
          ...
          >
            <option value="">🔴 Player Coming OFF *</option>
          ...
          >
            ...
            Execute Sub ({currentMatchMinute}')
          </button>
        </form>
        ...
        <form onSubmit={handleAddCard} className="p-4 rounded-2xl bg-[#090D16] border border-[#1E293B] space-y-3">
          ...
          >
            ...
            {match.roster.map((p) => (
              <option key={p.userId} value={p.userId}>
                {p.name} ({p.team === 'green' ? 'Green' : 'Blue'})
              </option>
            ))}
          </select>
          ...
          <input
            type="text"
            placeholder="Reason (e.g. Tactical Foul)"
            value={cardReason}
      ...
      <div className="p-5 rounded-2xl bg-[#090D16] border border-[#1E293B] space-y-3">
        ...
        <div className="space-y-2 max-h-48 overflow-y-auto">
          {(!match.goals?.length && !match.substitutions?.length && !match.cardEvents?.length) && (
            <p className="text-xs text-slate-500 italic py-2 text-center">No match events logged yet. Kick off the match!</p>
          )}
          ...
          {match.goals?.map((g, i) => (
            <div key={`g_${i}`} className="flex items-center justify-between p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-xs">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-white">GOAL! {g.scorerName}</span>
                {g.assistName && <span className="text-slate-400">(Assist: {g.assistName})</span>}
              </div>
            ...
            <div key={s.id} className="flex items-center justify-between p-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-xs">
              <div className="flex items-center gap-2">
                ...
                <span className="text-slate-300">
                  <strong className="text-emerald-400">IN:</strong> {s.playerInName} ↔ <strong className="text-rose-400">OUT:</strong> {s.playerOutName}
                </span>
              </div>
          ...
          {match.cardEvents?.map((c) => (
            <div key={c.id} className="flex items-center justify-between p-2.5 rounded-xl bg-slate-800/80 border border-slate-700 text-xs">
              ...
              <span className="text-[10px] text-slate-400 uppercase font-bold">{c.type}</span>
            </div>
          ))}
        </div>
      </div>
